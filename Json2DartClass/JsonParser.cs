...
namespace Json2DartClass
{
    public class JsonParser
    {
        ...
        Stack<JsonField> stack = new Stack<JsonField>();
        string tmp_name="";
        ...
        public static JsonField parse(string jsonString)
        {
            return new JsonParser().Parse(jsonString);
        }

        public JsonField Parse(string text)
        {
                ...
                switch (state)
                {
                    ...
                    case 3:
                        ...
                        else if (Regex.IsMatch(token.Value, number_pattern))
                        {
                            ...
                            if (stack.Count == 0)
                            {
                                stack.Push(new JsonNumber(Convert.ToDouble(resetValue)));
                            }
                            else if (stack.Peek() is JsonObject)
                            {
                                ((JsonObject)stack.Peek()).add(tmp_name, Convert.ToDouble(resetValue));

                            }
                            else if (stack.Peek() is JsonArray)
                            {
                                ((JsonArray)stack.Peek()).add(Convert.ToDouble(resetValue));
